...
model_s = load_model("app/static/model/mnist_Adam2_20210516.h5") # <- model_name.h5 is the name of the model that you made
path = ""; secure_files=[]
...
def upload_file():
    ...
    if request.method=='POST' and form.validate_on_submit():
        ...
        if file and allowed_file(file.filename):
            ...
            form=ResultForm()
            return render_template("results.html",title="Result",form=form,path=os.path.join('static/img/upload/', new_filename),result=result[0],pred=pred)
        else:
